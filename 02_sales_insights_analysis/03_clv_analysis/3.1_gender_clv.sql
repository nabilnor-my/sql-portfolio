...
WITH step1_joined_data AS (
    ...
    u.gender,                               -- segmentation variable (dimension)
    u.state,                                -- (future segmentation option)
...
-- Final aggregation: roll user-level CLV up to gender segments
SELECT
  gender,
  COUNT(user_id) AS total_customer,
  -- how many customers in segment
  ROUND(SUM(user_revenue) / SUM(user_order_count),2) AS gender_aov,
  -- overall segment AOV (revenue ÷ orders)
  ROUND(SUM(user_revenue) / COUNT(user_id),2) AS avg_revenue_per_customer,
  ...
  ROUND(SUM(user_revenue),2) AS gender_revenue,
  -- total revenue by gender
  ROUND(SUM(user_clv),1) AS gender_clv,
  -- total segment CLV (sum of individuals)
  ROUND(AVG(user_revenue),2) AS avg_gender_revenue,
  -- sanity check: mean revenue per customer
  ROUND(AVG(user_clv),1) AS avg_gender_clv
  -- mean CLV per customer in this gender group
FROM step5_user_clv
GROUP BY gender
